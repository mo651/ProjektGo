	...
	// Wird ein Geländefeld auf dem Spielfeld gewählt:	
		...
		if taste == 1 && status == 1 { //linke Maustaste gerade gedrückt
			if mausX<1000 && mausY<600 {				// XXX wenn Variabel hier und im folgenden Spielfeldmethoden
				...
				s.SetzeGelände (x,y,einzufgel)			// XXXX Irgendwie hatte ich da einen Knoten im Kopf.....nochmal gucken, dass die Zuweisung stimmt, hier verdreht notwendig :-(
				/*for _,w := range s {					//XXX Direkter Zugriff nun möglich, da Matrix eingebaut
...
//	Editiermodus (nicht speichern) XXX muss noch geändert werden, da auch geladen werden soll		
		if !speichern{
			...
			if laden{
			//var level dateien.Datei 					// XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXx wieder einfügen, wenn Laden und Speichern möglich
			gfx.UpdateAus()
...
/*
				...
				for !level.Ende(){
					...
					i++
					fmt.Println(i-1,bs)	
					//dummyspielfeld[i-1].Dekodieren(bs)				
					}
				level.Ende()	*/									//XXX Laden noch nicht möglich; Dekodieren Methode fehlt noch
				laden=false
...
//Spielraster wird gezeichnet		
			...
			}*/								// XXXxXX Entfällt, da es eigene Zeichenmethode für das Spiefold gibt
			gfx.UpdateAn()
		}else{
			gfx.UpdateAus()
//Rechteck für das Speichern-Menü		
...
//Datei schreiben
			/*if len(erg)>0{
				...
				level.Ende()
			}*/										//XXXXXX Schreiben noch nicht möglich, da Kodieren Methode bei Spielfeld fehlt		
			speichern=false
		}
	}	
}
