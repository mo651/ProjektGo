...
func (s *data)	GibBreite () (x uint16) {
return (*s).x
}


func (s *data)	GibHoehe () (y uint16) {
return (*s).y
}
...
/*
func (s *data) GibReichweite(einheit einheiten.Einheit)(erg,erg2 [][]bool) {
	...
	erg2=make ([][]bool,(*s).y)
	for i:=uint16(0);i<(*s).y;i++{
		erg2[i]= make([]bool,(*s).x) //standartmäßig Matrix mit false
	}
	...
	reichweite = func (x,y,schritte uint16){
        ...
        if (*s).einheitenmatrix[y][x+1].GibSpieler()!=einheit.GibSpieler() {
				  erg2[y][x+1]=true
        }
			}
			if (*s).einheitenmatrix[y][x-1] ==nil {
        ...
        if (*s).einheitenmatrix[y+1][x].GibSpieler()!=einheit.GibSpieler() {
				  erg2[y+1][x]=true
        }
			}
			if (*s).einheitenmatrix[y-1][x] ==nil {
