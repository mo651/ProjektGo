...
//	(*neu).bg 	  = untergrundtab[t].bg
	...
	(*neu).bonusb = untergrundtab[t].bonusb
	(*neu).belegt = false 	// Standardmäßig mit false belegt
	(*neu).typ=t
	return neu
}
...
func (g *data) GibBonusBewegung () int16 {
	return (*g).bonusa
}

// NOTWENDIGKEIT SETZEN???-------------------------------------------------------------	
	// Eff.: Der Bonus zum Bewegungsswert des Geländes beträgt nun n. 
func (g *data) SetzeBonusBewegung (n int16) {
	(*g).bonusa = n
}
...
func (g *data) SetzeTyp (t string) {
	...
	(*g).bonusb = untergrundtab[t].bonusb
	(*g).typ=t
}
